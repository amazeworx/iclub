						...
						if (is_array($meta_value) && !empty($meta_value)) {
							...
							if (isset($value['invert']) && $value['invert'] == true) {
								...
								foreach ($meta_value as $key => $saved_value) {
									...
									} else if ($details_list['fields'][$detail]['type'] == 'repeatable') {
...
$shortsummary = get_post_meta($post_id, '_short_summary', true);

$languages_list = get_the_terms($post_id, 'language');
$languages_string = join(', ', wp_list_pluck($languages_list, 'name'));

$countries_list = get_the_terms($post_id, 'listing_country');
$countries_string = join(', ', wp_list_pluck($countries_list, 'name'));

$industries_list = get_the_terms($post_id, 'listing_industry');
$industries_string = join(', ', wp_list_pluck($industries_list, 'name'));

$established = get_post_meta($post_id, '_established', true);

$teamsize = get_post_meta($post_id, '_team_size', true);

$techstacks_list = get_the_terms($post_id, 'listing_tech_stack');
$techstacks_string = join(', ', wp_list_pluck($techstacks_list, 'name'));
...
<div class="listing-desc-container">
	<?php if ($shortsummary) : ?>
		<div class="tw-mb-6">
			<?php echo esc_html($shortsummary) ?>
		</div>
	<?php endif ?>
	<div class="tw-grid tw-grid-cols-1 md:tw-grid-cols-3 tw-gap-4 md:tw-gap-4 tw-my-6">

		<?php if ($languages_string) : ?>
			<div class="flex flex-col">
				<div class="tw-text-[#999999] tw-text-[13px] tw-font-semibold tw-uppercase">Language</div>
				<div class="tw-text-base tw-font-semibold"><?php echo esc_html($languages_string) ?></div>
			</div>
		<?php endif; ?>

		<?php if ($countries_string) : ?>
			<div class="flex flex-col">
				<div class="tw-text-[#999999] tw-text-[13px] tw-font-semibold tw-uppercase">Country</div>
				<div class="tw-text-base tw-font-semibold"><?php echo esc_html($countries_string) ?></div>
			</div>
		<?php endif; ?>

		<?php if ($industries_string) : ?>
			<div class="flex flex-col">
				<div class="tw-text-[#999999] tw-text-[13px] tw-font-semibold tw-uppercase">Industry</div>
				<div class="tw-text-base tw-font-semibold"><?php echo esc_html($industries_string) ?></div>
			</div>
		<?php endif; ?>

		<?php if (!empty($established)) : ?>
			<div class="flex flex-col">
				<div class="tw-text-[#999999] tw-text-[13px] tw-font-semibold tw-uppercase">Established</div>
				<div class="tw-text-base tw-font-semibold"><?php echo esc_html($established) ?></div>
			</div>
		<?php endif; ?>

		<?php if (!empty($teamsize)) : ?>
			<div class="flex flex-col">
				<div class="tw-text-[#999999] tw-text-[13px] tw-font-semibold tw-uppercase">Team Size</div>
				<div class="tw-text-base tw-font-semibold"><?php echo esc_html($teamsize) ?></div>
			</div>
		<?php endif; ?>

		<?php if ($techstacks_string) : ?>
			<div class="flex flex-col">
				<div class="tw-text-[#999999] tw-text-[13px] tw-font-semibold tw-uppercase">Tech Stack</div>
				<div class="tw-text-base tw-font-semibold"><?php echo esc_html($techstacks_string) ?></div>
			</div>
		<?php endif; ?>
